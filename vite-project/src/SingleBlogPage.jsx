...
const SingleData=[
  ...
  return(
    <div>
         <img className='blog-image' src={blog.imageSrc} alt={blog.title}/>
         <div>
            <span className='blog-category'>{blog.category}</span>
            <h2 className='blog-title'>{blog.title}</h2>
            <p className='blog-excerpt'>{blog.excerpt}</p>
            {!IsChanged && (
                <button className='down-button' onClick={ClickChange}>Read More...</button>
            )}
            {IsChanged && <div className='blog-content'>{blog.fullContent}</div>}
         </div>
        </div>
...
function SingleBlogPage() {
    ...
    const[RowBlog,SetRowblog]=useState(3);

    const Viewall=()=>{
        SetRowblog(SingleData.length)
    }
const Viewblog=SingleData.slice(0,RowBlog);
    
  return (
    <div>
        <div className='TopHead'><h3 className='TopHead-Font'>Popular Post</h3>
        <button className='Viewall-button' onClick={Viewall}>View All</button></div>
        <div className='image-grid'>{Viewblog.map((blog)=>(
            <Single key={blog.id}{...blog}/>
        ))}</div>
